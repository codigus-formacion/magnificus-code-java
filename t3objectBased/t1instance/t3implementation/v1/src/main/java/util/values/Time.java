...
public class Time {
    ...
    public final int HOURS_MODULE = 24;
    public final int MINUTES_MODULE = 60;
    public final int SECONDS_MODULE = 60;
    ...
    public Time(int hours, int minutes, int seconds) {
        assert this.isValidHour(hours);
        assert this.isValidMinute(minutes);
        assert this.isValidSeconds(seconds);
    ...
    public boolean isValidHour(int hour) {
        return new IntegerInterval(0, this.HOURS_MODULE - 1).includes(hour);
    }

    public boolean isValidMinute(int minute) {
        return new IntegerInterval(0, this.MINUTES_MODULE - 1).includes(minute);
    }

    public boolean isValidSeconds(int seconds) {
        return new IntegerInterval(0, this.SECONDS_MODULE - 1).includes(seconds);
    }
